...
class Result extends CI_Controller {
	...
	function view( $id, $quid='' ) {
		...
		foreach ( $last_ten_result as $val ) {
			$value[]=array( $val['username'], floatval( $val['percentage'] ) );
		}
		...
		foreach ( explode( ",", $data['result']->time_spent_ind ) as $key => $val ) {
			if ( $correct_incorrect[$key]>="0.1" ) {
				$qtime[]=array( "Q ".( $key+1 ).") - Correct", floatval( $val ) );
			}else if ( $correct_incorrect[$key]==0 && $oidss[$key]!=0 ) {
					$qtime[]=array( "Q ".( $key+1 ).") - Incorrect ", floatval( $val ) );
			} else {
				$qtime[]=array( "Q ".( $key+1 ).") - UnAttempted ", floatval( $val ) );
			}

		}
		foreach ( $cct as $cck => $cckval ) {
			$ctime[]=array( $cck.' - Score: '.number_format( (float)( ( $cct_per[$cck]/$cct_per_total[$cck] )*100 ), 2, '.', '' )."%", floatval( $cckval ) );
		}
